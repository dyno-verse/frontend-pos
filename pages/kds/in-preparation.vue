  ...
  <div class="mx-auto">
    <div v-if="!isPending"
      ...
      <div v-if="orders.length !== 0" v-for="(order, index) in orders"
           class="bg-yellow-50 w-full h-full rounded-lg border border-yellow-500 p-5"
        ...
        <div v-for="o in order.orderItems" class="bg-yellow-50 border-dashed border border-yellow-400 rounded-lg my-2">
...
const updateOrderStatus = (orderId: string, position: number) => {
  ...
  const request: IKitchenStatus = {
    kitchenStatus: KitchenStatus.READY_TO_SERVE,
    itemIds: items
  }
